...
router.get('/', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT * FROM dunningsequences WHERE orgid = $1 ORDER BY name ASC',
      [req.auth!.orgId]
    );
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT * FROM dunningsequences WHERE id = $1 AND orgid = $2 LIMIT 1',
...
router.post('/', requireAuth, requireVerifiedEmail, async (req, res) => {
  ...
  try {
    ...
    if (d.isdefault) {
      await client.query(
        'UPDATE dunningsequences SET isdefault = false WHERE orgid = $1',
        [req.auth!.orgId]
      );
    }

    const { rows } = await client.query(
      `INSERT INTO dunningsequences (id, orgid, name, isdefault, steps)
       VALUES ($1,$2,$3,$4,$5)
       RETURNING *`,
      [id, req.auth!.orgId, d.name, d.isdefault ?? false, JSON.stringify(d.steps ?? [])]
    );
...
router.patch('/:id', requireAuth, requireVerifiedEmail, async (req, res) => {
  ...
  try {
    ...
    const sets: string[] = [];
    const values: unknown[] = [req.params.id, req.auth!.orgId];
    let idx = 3;
    ...
    const { rows } = await client.query(
      `UPDATE dunningsequences SET ${sets.join(', ')} WHERE id = $1 AND orgid = $2 RETURNING *`,
      values
    );
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    const { rowCount } = await pool.query(
      'DELETE FROM dunningsequences WHERE id = $1 AND orgid = $2',
      [req.params.id, req.auth!.orgId]
    );
