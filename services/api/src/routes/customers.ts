...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT * FROM customers WHERE id = $1 AND orgid = $2 LIMIT 1',
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `INSERT INTO customers (id, orgid, name, email, phone, whatsappphone, gstin, billingaddress, notes, preferredchannel)
...
router.patch('/:id', requireAuth, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE customers SET ${sets.join(', ')} WHERE id = $1 AND orgid = $2 RETURNING *`,
      values
    );
    if (!rows[0]) return res.status(404).json({ error: 'Customer not found' });
    res.json({ customer: rows[0] });
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    const { rowCount } = await pool.query(
      'DELETE FROM customers WHERE id = $1 AND orgid = $2',
      [req.params.id, req.auth!.orgId]
    );
