...
function verifyToken(req: Request): { sub: string; email: string; role: string } {
  const auth = req.headers.authorization;
  if (!auth?.startsWith('Bearer ')) throw Object.assign(new Error('No token'), { status: 401 });
...
function requireAuth(req: Request, res: Response, next: NextFunction) {
  try {
    (req as any).jwtPayload = verifyToken(req);
    next();
  } catch {
...
app.post('/auth/signin', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, email, full_name, company, password_hash FROM profiles WHERE email = $1 LIMIT 1',
      [email]
    );
...
app.post('/auth/signup', async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO profiles (id, email, full_name, company, password_hash, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`,
      [id, email, full_name ?? null, company ?? null, password_hash]
    );

    const token = jwt.sign(
      { sub: id, role: 'authenticated', email },
      JWT_SECRET,
      { expiresIn: JWT_EXPIRES_IN }
    );
...
app.post('/storage/sign-upload', requireAuth, async (req, res) => {
  if (!s3 || !R2_BUCKET) return res.status(500).json({ error: 'R2 not configured' });
  ...
  const userId = (req as any).jwtPayload.sub as string;
...
app.post('/api/razorpay-webhook', async (req, res) => {
  ...
  try {
    ...
    if (event.event === 'payment_link.paid' || event.event === 'payment.captured') {
      ...
      let paymentLinkRecord: any = null;
      if (razorpayLinkId) {
        const { rows } = await pool.query(
          `SELECT * FROM payment_links WHERE payment_reference = $1 LIMIT 1`,
          [razorpayLinkId]
        );
      ...
      if (!paymentLinkRecord && referenceId) {
        const { rows } = await pool.query(
          `SELECT * FROM payment_links WHERE notes = $1 LIMIT 1`,
          [referenceId]
        );
        paymentLinkRecord = rows[0] || null;
      }
      ...
      // Resolve userId
      let userId: string | undefined = paymentLinkRecord?.user_id;
      if (!userId && referenceId) {
        const match = referenceId.match(/(?:pro|premium)-(.+)/);
        if (match) userId = match[1];
      }
      if (!userId && paymentEntity?.notes?.user_id) userId = paymentEntity.notes.user_id;

      if (userId) {
        await pool.query(
          `INSERT INTO payment_transactions (user_id, amount, status, transaction_id, notes, created_at)
           VALUES ($1,$2,'verified',$3,$4,NOW())`,
          [userId, amount, paymentEntity?.id || razorpayLinkId, `Razorpay webhook ${event.event}`]
        );
      }
      ...
      if (planMatch) planType = planMatch[1] as 'pro' | 'premium';
      if (!planType) {
      ...
      if (planType && userId) {
        ...
        await pool.query(
          `INSERT INTO user_plans (user_id, plan, is_active, started_at, expires_at, ai_queries_limit)
           VALUES ($1,$2,true,NOW(),$3,$4)
           ON CONFLICT (user_id) DO UPDATE
             SET plan=EXCLUDED.plan, is_active=true, started_at=NOW(),
                 expires_at=EXCLUDED.expires_at, ai_queries_limit=EXCLUDED.ai_queries_limit`,
          [userId, planType, expiresAt.toISOString(), 999999]
        );

        await pool.query(
          `UPDATE payment_transactions SET processed=true
           WHERE user_id=$1 AND status='verified' AND (processed IS FALSE OR processed IS NULL)`,
          [userId]
        );
...
app.post('/api/reconcile-itc', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const { rows: planRows } = await pool.query(
      `SELECT plan, is_active FROM user_plans WHERE user_id = $1 LIMIT 1`,
      [userId]
    );
    ...
    for (const purchase of purchases) {
      ...
      if (upsertRows[0]) reconciliationResults.push(upsertRows[0]);
    }
