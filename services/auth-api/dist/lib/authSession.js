...
function buildSessionUser(user, membership) {
    return {
        id: user.id,
        email: user.email,
...
export async function loadPrimaryMembership(userId, client) {
    const q = client ?? pool;
    const { rows } = await q.query(`SELECT om.orgid, om.role, COALESCE(s.plan, 'free') AS plan
     FROM orgmembers om
...
export async function createSession(client, user, membership, req) {
    const rawRefresh = generateRefreshToken();
    const familyId = randomUUID();
    await client.query(`INSERT INTO refreshtokens (userid, tokenhash, familyid, expiresat, useragent, ip)
     VALUES ($1, $2, $3, $4, $5, $6)`, [
        user.id,
...
export function deliverSession(res, rawRefresh, payload) {
...
export async function rotateRefreshToken(rawToken, req, res) {
    ...
    try {
        ...
        const { rows } = await client.query(`SELECT id, userid, familyid, revokedat, expiresat
       FROM refreshtokens
       WHERE tokenhash = $1
       FOR UPDATE`, [tokenHash]);
        ...
        if (row.revokedat) {
            ...
            await client.query(`UPDATE refreshtokens SET revokedat = now()
         WHERE familyid = $1 AND revokedat IS NULL`, [row.familyid]);
            await client.query(`UPDATE users SET tokenversion = tokenversion + 1 WHERE id = $1`, [row.userid]);
        ...
        await client.query(`UPDATE refreshtokens SET revokedat = now() WHERE id = $1`, [row.id]);
        const userResult = await client.query(`SELECT id, email, fullname, emailverifiedat, tokenversion FROM users WHERE id = $1`, [row.userid]);
...
export async function revokeRefreshToken(rawToken, res) {
    const tokenHash = hashToken(rawToken);
    await pool.query(`UPDATE refreshtokens SET revokedat = now() WHERE tokenhash = $1 AND revokedat IS NULL`, [tokenHash]);
    clearRefreshCookie(res);
}
