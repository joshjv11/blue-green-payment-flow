...
async function razorpayWebhookHandler(req, res) {
    ...
    try {
        ...
        const existing = await pool.query('SELECT id FROM webhookevents WHERE eventid = $1 LIMIT 1', [eventId]);
        if (existing.rows[0]) {
            return res.json({ ok: true, duplicate: true });
        }
        await pool.query(`INSERT INTO webhookevents (provider, eventid, payload) VALUES ('razorpay', $1, $2::jsonb)`, [eventId, bodyText]);
        if (event.event === 'payment_link.paid' || event.event === 'payment.captured') {
            ...
            if (razorpayLinkId) {
                const { rows } = await pool.query(`SELECT id, orgid, invoiceid FROM paymentlinks WHERE providerreferenceid = $1 LIMIT 1`, [razorpayLinkId]);
            ...
            if (paymentLinkRecord) {
                ...
                try {
                    ...
                    await client.query(`UPDATE paymentlinks SET status = 'paid', paidat = NOW() WHERE id = $1 AND orgid = $2`, [paymentLinkRecord.id, paymentLinkRecord.orgid]);
                    const paymentInsert = await client.query(`INSERT INTO payments (orgid, invoiceid, amount, method, providerpaymentid, rawwebhook)
             ...
             RETURNING id`, [
               ...
               SET amountpaid = LEAST(amount + taxamount, amountpaid + $3),
                   status = CASE
                     ...
                     ELSE status
                   END
               WHERE id = $1 AND orgid = $2`, [paymentLinkRecord.invoiceid, paymentLinkRecord.orgid, amount]);
                    }
                    await client.query(`UPDATE webhookevents SET processedat = NOW() WHERE eventid = $1`, [eventId]);
