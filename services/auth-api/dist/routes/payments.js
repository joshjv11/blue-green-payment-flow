...
async function createPaymentLinkHandler(req, res) {
    ...
    try {
        ...
        if (gateway === 'upi') {
            const orgResult = await pool.query('SELECT upivpa FROM organizations WHERE id = $1 LIMIT 1', [orgId]);
        ...
        const { rows } = await pool.query(`INSERT INTO paymentlinks (orgid, invoiceid, gateway, url, providerreferenceid, amount, status, expiresat)
       VALUES ($1,$2,$3,$4,$5,$6,'active',$7)
       RETURNING *`, [orgId, invoiceid, gateway, paymentLinkUrl, providerReferenceId || null, amount, expiresAt.toISOString()]);
        res.json({
            success: true,
            paymentLink: {
                id: rows[0].id,
