...
import { createSession, deliverSession, loadPrimaryMembership, rotateRefreshToken, revokeRefreshToken, REFRESH_COOKIE, } from '../lib/authSession.js';
import { createVerificationToken, resetPasswordWithToken, verifyEmailWithToken, } from '../lib/verificationTokens.js';
import { sendVerificationEmail, sendPasswordResetEmail } from '../lib/email.js';
import { signinLimiter, signupLimiter, forgotPasswordLimiter, refreshLimiter, verifyEmailLimiter, } from '../middleware/rateLimit.js';
...
router.post('/signin', signinLimiter, async (req, res) => {
    ...
    try {
        const userResult = await pool.query(`SELECT id, email, fullname, passwordhash, emailverifiedat, tokenversion
...
router.post('/signup', signupLimiter, async (req, res) => {
    ...
    try {
        ...
        await client.query(`INSERT INTO users (id, email, fullname, passwordhash) VALUES ($1, $2, $3, $4)`, [userId, email, full_name ?? null, passwordhash]);
        await client.query(`INSERT INTO organizations (id, name) VALUES ($1, $2)`, [orgId, orgName]);
        await client.query(`INSERT INTO orgmembers (orgid, userid, role) VALUES ($1, $2, 'owner')`, [orgId, userId]);
        await client.query(`INSERT INTO subscriptions (orgid, plan, status) VALUES ($1, 'free', 'active')`, [orgId]);
...
router.get('/me', requireAuth, async (req, res) => {
    try {
        ...
        const userResult = await pool.query(`SELECT id, email, fullname, emailverifiedat FROM users WHERE id = $1`, [req.auth.userId]);
        const user = userResult.rows[0];
        if (!user || !membership) {
            return res.status(404).json({ error: 'User not found' });
        }
        const orgResult = await pool.query(`SELECT id, name, gstin, udyamnumber, upivpa, address, logourl FROM organizations WHERE id = $1`, [req.auth.orgId]);
...
router.patch('/me', requireAuth, async (req, res) => {
    ...
    try {
        ...
        await client.query('COMMIT');
        const membership = await loadPrimaryMembership(req.auth.userId);
        const userResult = await pool.query(`SELECT id, email, fullname, emailverifiedat, tokenversion FROM users WHERE id = $1`, [req.auth.userId]);
        const user = userResult.rows[0];
        if (!user || !membership) {
            return res.status(404).json({ error: 'User not found' });
        }
