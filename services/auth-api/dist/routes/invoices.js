...
router.get('/:id', requireAuth, async (req, res) => {
    try {
        const { rows } = await pool.query(`SELECT i.*, c.name AS customername
       FROM invoices i
       JOIN customers c ON c.id = i.customerid AND c.orgid = i.orgid
       WHERE i.id = $1 AND i.orgid = $2
       LIMIT 1`, [req.params.id, req.auth.orgId]);
        if (!rows[0])
...
router.post('/', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const { rows } = await pool.query(`INSERT INTO invoices (id, orgid, customerid, invoicenumber, issuedate, duedate, amount, taxamount, currency, lineitems, ismsmesupplier, msmeagreementexists, status)
...
router.patch('/:id', requireAuth, async (req, res) => {
    ...
    try {
        const { rows } = await pool.query(`UPDATE invoices SET ${sets.join(', ')} WHERE id = $1 AND orgid = $2 RETURNING *`, values);
        if (!rows[0])
            return res.status(404).json({ error: 'Invoice not found' });
        res.json({ invoice: rows[0] });
    }
...
router.delete('/:id', requireAuth, async (req, res) => {
    try {
        const { rowCount } = await pool.query(`DELETE FROM invoices WHERE id = $1 AND orgid = $2 AND status = 'draft'`, [req.params.id, req.auth.orgId]);
        if (!rowCount)
            return res.status(404).json({ error: 'Invoice not found or not deletable' });
        res.json({ ok: true });
    }
    catch (err) {
