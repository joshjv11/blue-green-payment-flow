-- Enhanced security for payment transactions - Fixed version
...
-- Create audit trigger for admin user creation
...
BEGIN
  ...
  INSERT INTO public.payment_access_log (
    ...
    'admin_user_created',
    'New admin user created with role: ' || COALESCE(NEW.role, 'admin') || ', status: ' || COALESCE(NEW.status, 'pending')
  );
...
-- Create trigger for admin creation audit
DROP TRIGGER IF EXISTS admin_creation_audit ON public.admin_users;
